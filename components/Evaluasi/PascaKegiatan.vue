...
export default {
  ...
  methods: {
    ...
    Evaluasi() {
      ...
      const url = `${this.api_url}/web/kegiatan/evaluasi-pasca-kegiatan/list?kegiatan_id=${this.kegiatan_id}`;
      this.$axios.defaults.headers.common.Authorization = `Bearer ${this.token.accessToken}`;
      this.$axios
        .get(url)
        .then(({ data }) => {
    ...
    KirimJawaban() {
      window.scrollTo(0, 0);
      const url = `${this.api_url}/web/kegiatan/evaluasi-pasca-kegiatan/jawaban`;
      this.$axios.defaults.headers.common.Authorization = `Bearer ${this.token.accessToken}`;
      ...
      this.lists.forEach((entry) => {
        paramJawaban.push({
          ...
          pelatihan_evaluasi_pasca_kegiatan_id: entry.id,
